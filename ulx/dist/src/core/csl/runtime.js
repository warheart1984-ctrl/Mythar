...
function hashedId(prefix, parts) {
...
function matchesRule(rule, intent) {
...
export class RuntimeEngine {
    ...
    generateIntent(agentId, kind, context = {}) {
    ...
    selectPolicy(intent) {
        ...
        const matching = sorted.find((policy) => policy.enabled &&
    ...
    deriveConstraints(policy, intent) {
    ...
    resolveDecision(intent, agent, policy, constraints) {
        ...
        const outcome = blocked ? "blocked" : escalated ? "escalated" : accepted ? "accepted" : "blocked";
    ...
    runOnce(agentId, kind, context = {}) {
        ...
        const lineage = [
            ...
            ...constraints.map((constraint) => this.ciems.record_lineage({
    ...
    runLoop(iterations, kind, context = {}) {
        if (iterations <= 0) {
            return [];
        }
        const results = [];
