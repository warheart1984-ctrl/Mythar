...
class MytharClient:
    ...
    def compile(self, expression: str, mode: str = "strict") -> dict:
        headers = {"Content-Type": "application/json"}
        if self.api_key: headers["X-API-Key"] = self.api_key
        request = Request(f"{self.base_url}/v1/compile", data=json.dumps({"expression": expression, "mode": mode}).encode(), headers=headers, method="POST")
        try:
